<template>
	<view class="container">
		...
		<view class="empty-container" v-if="orders.length === 0">
			...
			<text class="empty-text">暂无未收货订单</text>
		</view>
			...
			<view class="order-card" v-for="(item, index) in orders" :key="item.id">
				<view class="order-header">
					...
					<view class="order-status">
						<text class="status-tag">未收货</text>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			getNoDeliveryOrders() {
				...
				uni.request({
					url: `${baseUrl}/api/user/order/noreceive`,
					method: 'GET',
					header: {
						'authorization': token
					},
					success: (res) => {
						...
						if (res.data.code === 1) {
							...
							console.log('未收货订单数据:', this.orders);
						} else {
							uni.showToast({
								title: '获取订单失败',
								icon: 'none',
