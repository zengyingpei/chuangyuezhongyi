...
export class GameMap extends AcGameObjects {
        ...
        if(this.store.state.record.is_record){
            ...
            const interval_id=setInterval(() => {
                if(k>=a_step.length-1){
                    ...
                    snake0.set_direction(a_step[k]);
                    snake1.set_direction(b_step[k]);
                }
                k++;
            }, 300);
        }else{
    ...
    render(){
        // 按照奇数和偶数将格子染成不同颜色
        const color_even="#9DA3BD", color_odd="#C3C0D3";
        for(let row=0; row<this.rows; row++){
            for(let col=0; col<this.cols; col++){
                ...
                this.ctx.fillRect(this.L * col, this.L * row, this.L, this.L);
            }
        }
    }
}
